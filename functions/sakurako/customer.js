...
$(document).ready(() => {
	...
	var listAllUsers = functions.httpsCallable('listAllUsers');
	...
	function updateTableRow(userRecord) {
		...
		if (userRecord.photoURL !== null) {
			...
			}else if (userRecord.photoURL.includes("googleusercontent.com")) {
				subImage = userRecord.photoURL.replace("s96-c", "s400-c").trim();
			}else {
				subImage = userRecord.photoURL;
			}
			console.log(subImage);
			image = '<img class="image profile_img" name="'+userRecord.displayName+'" src="'+ subImage +'" width="70" height="70" />';			
		}else {
		...
		var dataset = [image, userRecord.displayName || 'undefined', userRecord.email || '<center>--</center>', (userRecord.customClaims.rating || 0), (userRecord.customClaims.cancelled || 0), viewIcons, mdc_button];
